...
### Changelog
- **v0.4.0 (2026-06-25)** — Add **machine-readable conformance** (SHOULD, Self-description): a tool
  declares the conformed Guidelines `version` + `level` in its `schema` output, so an agent (and a
  fleet audit) can verify the contract version it's driving — not just a human-readable README badge.
  *Assumption:* agents introspect tools at runtime and benefit from a machine-readable contract claim.
...
| [Machine-readable conformance in `schema`](/self-description/#schema--the-machine-readable-surface) | Agents introspect a tool at runtime and benefit from reading the conformed contract version, not a human badge. | Durable while agents introspect tools at runtime. |
